...
statuses = ["WhQKCAoGb25saW5lEgYKBHBvb3AaAA==", "Wh0KBQoDZG5kEhIKB3Bvb3BpbmchgH0r1IgBAAAaAA=="]
statuses_enumerator = 0

while True:
    payload = json.dumps({
        "settings": statuses[statuses_enumerator]
    })
    status_change_response = requests.request("PATCH", url, headers=headers, data=payload)
    operation_success = status_change_response.status_code == 200
    name_changed_string = f'Status changed to {statuses[statuses_enumerator]}' if operation_success else 'Status not changed'
    print(f'{name_changed_string}. Status code: {status_change_response.status_code}')  # Print the response status code
    if operation_success:
        statuses_enumerator += 1
    if statuses_enumerator == len(statuses):
        statuses_enumerator = 0
